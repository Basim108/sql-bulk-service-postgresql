    ...
    public class NpgsqlCommandsBulkService : IPostgreSqlBulkService
    {
            ...
            where TEntity : class
        {
            ...
            else
            {
                ...
                if (sw != null)
                {
                    _logger.LogInformation($"{operation}ed {elements.Count} {entityProfile.EntityType.Name} elements in {iterations} steps in total {sw.ElapsedMilliseconds / 1000:F3} seconds");
                }
            }
            return result;
        }
            ...
            where TEntity : class
        {
            ...
            {
                sw.Stop();
                var cmdSize = generatedCommands.Max(x => x.Command.Length) * 2;
                var suffix  = "bytes";
                if (cmdSize > 1024)
                {
                ...
                _logger.LogInformation(
                    $"Generated {generatedCommands.Count} sql {operation.ToString().ToLowerInvariant()} commands for {elements.Count} {entityProfile.EntityType.Name} elements in {sw.ElapsedMilliseconds} ms, max command size {cmdSize:F2} {suffix}");
                sw = Stopwatch.StartNew();
            }
            ...
            if (sw != null)
            {
                _logger.LogInformation($"{operation}ed a portion of {elements.Count} {entityProfile.EntityType.Name} elements in {sw.ElapsedMilliseconds / 1000:F3} seconds");
            }
        }
