...
namespace Hrimsoft.SqlBulk.PostgreSql.IntegrationTests.ErrorManagement
{
    public class StopEverythingStrategyTests
    {
        ...
        [Test]
        public async Task Exception_should_have_only_problem_and_operated_elements()
        {
            var bulkServiceOptions = new BulkServiceOptions
            {
                FailureStrategy = FailureStrategies.StopEverything,
            ...
            using (var connection = new NpgsqlConnection(_configuration.ConnectionString))
            {
                ...
                Assert.IsNotEmpty(ex.OperatedElements);
                Assert.NotNull(ex.OperatedElements.First());
                Assert.AreEqual("rec-0", ex.OperatedElements.First().RecordId);
            }
        }
